...
vector<int> exchangeServer(vector<int> curServer, int exStep)
{
    int vis[netCount];
    ...
    for(int i = 0; i < exStep - 1; i ++)
    {
        while(vis[temp] == 1)
            temp = (temp + 1) % netCount;
        vis[temp] = 1;
        selectServer = (selectServer + 1) % curServerLen;
        vis[curServer[selectServer]] = 0;
        curServer[selectServer] = temp;
    }
...
//方式2：从已知的服务器节点中随机去掉1个节点
vector<int> deleteServer(vector<int> curServer)
{
    int vis[netCount];
    memset(vis, 0, sizeof(vis));
...
{
    int exStep = 2;
    if(netCount > 100 && netCount < 300) exStep = 3;
    else if(netCount >= 300 && netCount <= 500) exStep = 5;
    else exStep = 7;
    return exStep;
}
    ...
    while (annealc.temp > minTemp)				//当温度衰减到一定程度，认为已经达到平衡状态,视当前最优解为全局最优解
    {
        for (int i = 0; i < markovLen; i += 1)
        {
            ...
            if(annealc.curServer.size() >= threshold)
            {
                ...
                else annealc.nextServer = deleteServer(annealc.curServer);
            }
            else
            {
                ...
                //cout << selectFunc << endl;
                if(selectFunc == 0) annealc.nextServer = exchangeServer(annealc.curServer);
                else if(selectFunc == 1) annealc.nextServer = deleteServer(annealc.curServer);
                else annealc.nextServer = addServer(annealc.curServer);
            }
            */
            int selectFunc = rand() % 3;
            //cout << selectFunc << endl;
            if(selectFunc == 0) annealc.nextServer = exchangeServer(annealc.curServer, exStep);
            else if(selectFunc == 1) annealc.nextServer = deleteServer(annealc.curServer);
            else annealc.nextServer = addServer(annealc.curServer);
